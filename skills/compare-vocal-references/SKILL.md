...
如果用户已经要求分析，但没有提供完整输入，一次性引导其补充：

- 一条待评估人声的附件或绝对路径。
- 至少一条参考人声的附件或绝对路径。
- 哪些参考属于 Source Separation（人声分离）。
- 可选的报告输出目录。

当用户没有指定报告输出目录时，引导消息必须同时说明默认落点，不能只询问音频。先检查默认目录并给出实际将使用的绝对路径。默认话术结构：

> 请提供一条待评估人声和至少一条参考人声，并说明哪些参考来自人声分离。你也可以指定报告输出目录；如果不指定，我会把本次报告、图表和指标保存到 `C:\Projects\work\<本次任务子目录>`。

如果 `C:\Projects\work` 不可用，把示例中的路径替换成已选定的任务独立目录。只缺少输出路径时不需要停下等待确认；明确告知实际默认路径后继续分析。

### 2. 准备确定性的分析环境

选择输出目录时，若 `C:\Projects\work` 存在且可写，默认将其作为 `--out-dir`；为避免不同任务的同名文件互相覆盖，在其中创建清晰命名的任务子目录。若该路径不存在或不可写，再使用当前任务的独立输出目录。用户未指定路径时，必须在运行分析器前用一句话明确告知最终选定的绝对输出路径。绝不覆盖源音频。
...
分析结束后，必须把最终报告直接交付到 Codex 对话中，不能只提供文件路径或要求用户自行打开 Markdown：

1. 确认 `reference-set-report.md` 已经是最终整理版，而不是未校对的脚本草稿；新增的重要解释也必须写回该文件。
2. 完整读取最终 `reference-set-report.md`。
3. 以 `## 报告正文` 开始，按原有 Markdown 结构完整呈现报告正文、表格和结论；不得只贴摘要、节选或“请打开文件查看”。
4. 报告正文和图片全部呈现完毕后，再以 `## 输出文件` 列出输出目录、报告、指标、执行日志和主要图片的绝对路径。
5. 不要把报告正文放进代码块。报告中的图片使用绝对路径直接显示；相对图片路径应在对话展示时改成对应的绝对路径，确保 Codex 可以预览。
6. 即使用户本地没有 Markdown 查看器，单独阅读最终回复也必须能获得完整报告。
